 ...
 * 
 * Copyright (C) 2004-2011 by Aric Hagberg <hagberg@lanl.gov> Dan Schult
 * <dschult@colgate.edu> Pieter Swart <swart@lanl.gov> All rights reserved. BSD
 * license.
 */
...
function betweenness_centrality(G, opts) {
	var k=opts.k || null, normalized=opts.normalized || true,
	weight=opts.weight || null, endpoints=opts.endpoints || false,
	seed=opts.seed || null;
 ...
 * all-pairs shortest paths that pass through `e`: .. math::
 * 
 * c_B(v) =\sum_{s,t \in V} \frac{\sigma(s, t|e)}{\sigma(s, t)}
 * 
 * where `V` is the set of nodes,`\sigma(s, t)` is the number of shortest
 * `(s, t)`-paths, and `\sigma(s, t|e)` is the number of those paths passing
 * through edge `e` [2]_.
 * 
 * @param G
 ...
 *            bool, optional If True the betweenness values are normalized
 *            by `2/(n(n-1))` for graphs, and `1/(n(n-1))` for directed
 *            graphs where `n` is the number of nodes in G.
 * 
 * @param opt.weight
 *            None or string, optional If None, all edge weights are
 *            considered equal. Otherwise holds the name of the edge
 *            attribute used as weight.
 * 
 * @return edges : dictionary Dictionary of edges with betweenness
 ...
 * 
 * For weighted graphs the edge weights must be greater than zero. Zero edge
 * weights can produce an infinite number of equal length paths between
 * pairs of nodes.
 * 
 * References ---------- .. [1] A Faster Algorithm for Betweenness
 * Centrality. Ulrik Brandes, Journal of Mathematical Sociology
 * 25(2):163-177, 2001.
 * http://www.inf.uni-konstanz.de/algo/publications/b-fabc-01.pdf .. [2]
 * Ulrik Brandes: On Variants of Shortest-Path Betweenness Centrality and
 * their Generic Computation. Social Networks 30(2):136-145, 2008.
 * http://www.inf.uni-konstanz.de/algo/publications/b-vspbc-08.pdf
 * 
 * @export
 */
function edge_betweenness_centrality(G,opts){
	var normalized=opts.normalized || true,
...
function _single_source_shortest_path_basic(G,s){
	var S=[], 
	P= new goog.structs.Map(),
	Q =  [],
	D= new goog.structs.Map(),
	sigma = new goog.structs.Map(),
	v,
	Dv;
	// P[v]=[] for v in G
...
function _single_source_dijkstra_path_basic(G,s,weight){
	...
	Q.enqueue(0,{distance:0,pred:s,node:s});
	...
	while (Q.length > 0){
		...
		for (e in G.edges([v],true)){
			...
			if (D.containsKey(w) && 
					(!wseen || vw_dist < wseen)){
				seen.set(w, vw_dist);
				Q.enqueue(vw_dist,{dist:vw_dist,pred:v,node:w});
				sigma[w]=0.0;
				P[w]=[v];
...
function _accumulate_basic(betweenness,S,P,sigma,s){
	var delta= new goog.structs.Map(), 
	w, 
	coeff;
